...
import { edit } from '@/app/actions/blog';
import { publish } from '@/app/actions/blog';


export async function editDraft(editor: ReturnType<typeof useEditor>, title: string,{ params }: { params: Promise<{ blogId: string }> }) {
        ...
        <>
            <div className='flex flex-col items-center w-full gap-20 h-96 p-10'>
                ...
                <div className='flex flex-row gap-10'>
                    <Button onClick={() => editDraft(editor, title , {params})}>
                        Save
                    </Button>
                    <Button onClick={() => publish(editor, title)}>
                        Publish
                    </Button>
                </div>
            </div>
